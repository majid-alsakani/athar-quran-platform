...
import WeeklyPdfButton from "@/components/WeeklyPdfButton";
...
export default function ProgressPage() {
  ...
  const trend = trpc.reports.studentTrend.useQuery({ studentId: studentId ?? 0, weeks: 8 }, { enabled: Boolean(studentId) });
  const records = report.data?.progress ?? [];
  const attendance = report.data?.attendance ?? [];
  const present = attendance.filter(item => item.status === "present").length;

  const firstError = overview.error ?? members.error ?? circles.error ?? teacherStudents.error ?? report.error ?? trend.error;
  if (firstError) {
    return <AppFrame eyebrow="تقدم الطالب" title="تعذر تحميل التقدم" description="تحقق من اتصالك ثم أعد المحاولة."><QueryError message={firstError.message} onRetry={() => { overview.refetch(); members.refetch(); circles.refetch(); teacherStudents.refetch(); report.refetch(); trend.refetch(); }} /></AppFrame>;
  }
  ...
  return (
    <AppFrame eyebrow="تقدم الطالب" title="سجل الحفظ والمراجعة" description="تقارير مبسطة تظهر فقط للطالب نفسه، أو ولي أمره المرتبط، أو فريق المؤسسة المخول.">
      ...
      {report.isLoading ? <div className="grid place-items-center rounded-3xl bg-white py-16"><Loader2 className="size-7 animate-spin text-[#276654]" /></div> : studentId ? <><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><StatCard label="حضور هذا الأسبوع" value={`${present}/${attendance.length}`} icon={CalendarCheck2} /><StatCard label="سجلات متابعة" value={records.length} icon={BookMarked} tone="sand" /><StatCard label="نقاط مكتسبة" value={records.reduce((sum, item) => sum + item.pointsAwarded, 0)} icon={Award} tone="ink" /></div><StudentTrendChart data={trend.data ?? []} loading={trend.isLoading} /><section className="rounded-3xl border border-[#dfe8e1] bg-white p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-center gap-3"><span className="flex size-10 items-center justify-center rounded-xl bg-[#ecf4ed] text-[#2b6d59]"><Sparkles className="size-5" /></span><div><h2 className="font-display text-xl font-semibold text-[#193a31]">آخر المتابعات</h2><p className="text-sm text-[#718078]">الحفظ والمراجعة ومستوى التجويد كما سجلها المعلم.</p></div></div>{report.data ? <WeeklyPdfButton studentName={students.find(student => student.id === studentId)?.name ?? "الطالب"} weekStart={report.data.weekStart} present={present} attendanceTotal={attendance.length} records={records} /> : null}</div><div className="mt-5 space-y-3">{records.length ? records.map(record => <article key={record.id} className="rounded-2xl border border-[#e7eee9] p-4"><div className="flex flex-col justify-between gap-2 sm:flex-row"><div><p className="font-semibold text-[#29483e]">{record.memorizationFrom || record.memorizationTo ? `الحفظ: ${record.memorizationFrom ?? ""} ${record.memorizationTo ? `— ${record.memorizationTo}` : ""}` : "متابعة تعلم"}</p><p className="mt-1 text-sm text-[#738179]">{record.revisionTo ? `المراجعة: ${record.revisionTo}` : "لا توجد مراجعة مسجلة"}{record.notes ? ` · ${record.notes}` : ""}</p></div><div className="text-left"><span className="rounded-full bg-[#edf5ef] px-3 py-1 text-xs font-semibold text-[#2e6c58]">{record.tajweedGrade === "excellent" ? "ممتاز" : record.tajweedGrade === "very_good" ? "جيد جداً" : record.tajweedGrade === "good" ? "جيد" : "يحتاج دعماً"}</span><p className="mt-2 text-xs text-[#849088]">{new Date(record.recordedAt).toLocaleDateString("ar-SA")}</p></div></div></article>) : <div className="rounded-2xl bg-[#fafcfb] py-10 text-center text-sm text-[#77837c]">لا توجد سجلات متابعة لهذا الأسبوع بعد.</div>}</div></section></> : <div className="rounded-3xl border border-dashed border-[#c9d9ce] bg-[#fbfdfb] py-16 text-center text-sm text-[#77837c]">اختر طالباً لعرض التقدم.</div>}
    </AppFrame>
  );
}
